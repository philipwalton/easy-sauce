...
class EasySauce {
  ...
  runTests() {
    return this.validateInput()
        ...
        .then((jobs) => {
          ...
          let status = this.reportResults(jobs);
          return status;
        });
  }
      ...
      request.post(opts, (err, response, body) => {
        ...
        if (body.error) {
          return reject(new Error(
              messages.JOBS_ERROR + `(${response.statusCode}) ${body.error}`));
        }
      ...
      let checkJobStatus = () => {
        setTimeout(() => {
          ...
          request.post(params, (err, response) => {
            if (err) return reject(err);
            let jobs = response.body['js tests'];
  ...
  reportResults(jobs) {
    ...
    if (failures === 0) {
      ...
      this.logger.log(`Oops! There were ${failures} failures!`);
      return 1;
    }
  }
