...
class EasySauce {
  ...
  createTunnel() {
    return new Promise((resolve, reject) => {
      ngrok.connect(this.opts.port, (err, baseUrl) => {
        ...
        this.logger.emit('message', messages.TUNNEL_CREATED + this.opts.port);
  ...
  updateJobProgress(jobs) {
    for (let job of jobs) {
      ...
      let status = job.status || 'test finished';

      if (status != this.progress[job.id].status) {
        this.progress[job.id].status = status;
        this.logger.emit(
            'message', `${formatPlatform(job.platform)} : ${status}`);
      }
  ...
  reportResults(jobs) {
    this.logger.emit(
        'done', !jobs.some((job) => job.result.failures > 0), jobs);
  }
