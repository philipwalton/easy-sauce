...
const sauceConnectLauncher = require('./sauce-connect');
...
const NGROK = 'ngrok';
const SAUCE_CONNECT = 'sauce-connect';
...
class EasySauce {
  ...
  constructor(overrides) {
    ...
    this.opts.sauceConnect = Object.assign({
      accessKey: this.opts.key,
      username: this.opts.username
    }, this.opts.sauceConnect);
  ...
  createTunnel() {
    switch (this.opts.tunnel) {
      case NGROK:
        return this.runNgrok();
      case SAUCE_CONNECT:
        return this.runSauceConnect();
      default:
        throw new Error(messages('WRONG_TUNNEL_NAME', this.opts.tunnel));
    }
  }
  ...
  runNgrok() {
    return new Promise((resolve, reject) => {
      ngrok.connect(this.opts.port, (err, baseUrl) => {
        ...
        this.logger.emit(
          'message', messages('TUNNEL_CREATED', this.opts.port, this.baseUrl));
  ...
  runSauceConnect() {
    return new Promise((resolve, reject) => {
      sauceConnectLauncher(this.opts.sauceConnect,
        (err, sauceConnectProcess) => {
          this.sauceConnectProcess = sauceConnectProcess;
          if (err) return reject(err);
          this.baseUrl = `localhost:${this.opts.port}`;
          this.logger.emit(
            'message', messages('TUNNEL_CREATED',
              this.opts.port, this.baseUrl));
  ...
  startJobs() {
    return new Promise((resolve, reject) => {
      let url = path.normalize(`${this.baseUrl}/${this.opts.tests}`);
      let opts = {
  ...
  destroy() {
    // TODO: does this affect multiple instances?
